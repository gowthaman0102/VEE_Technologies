...
function getStatusClasses(status: string | null | undefined) {
  const safe = (status ?? "").toLowerCase();
  if (safe.includes("failed") || safe.includes("error")) {
    return "border-critical-border bg-critical-bg text-critical";
  }
  if (safe.includes("pending")) {
    return "border-medium-border bg-medium-bg text-medium";
  }
  if (safe.includes("processing")) {
    return "border-primary-border bg-primary-soft text-primary";
  }
  if (safe.includes("success")) {
    return "border-low-border bg-low-bg text-low";
  }
                ...
                <svg viewBox="0 0 190 190" className="h-full w-full" aria-hidden="true">
                  <defs>
                    <linearGradient id="reportPaper" x1="0%" x2="100%" y1="0%" y2="100%">
                      ...
                      <stop offset="100%" stopColor="#edf5f4" />
                    </linearGradient>
                  </defs>

                  <g opacity="0.72">
                    <rect x="26" y="30" width="98" height="128" rx="12" fill="#dfecee" transform="rotate(-9 75 94)" />
                    <rect x="52" y="18" width="98" height="128" rx="12" fill="#edf7f5" transform="rotate(7 101 82)" />
                  </g>

                  <g transform="translate(28 10) rotate(-6 67 95)">
                    <rect x="0" y="0" width="100" height="142" rx="12" fill="url(#reportPaper)" stroke="#dfeae9" />
                    <rect x="16" y="18" width="68" height="10" rx="4" fill="#e3efee" />
                    <rect x="16" y="34" width="48" height="8" rx="4" fill="#e9f4f3" />

                    <rect x="16" y="56" width="18" height="48" rx="5" fill="#0e5a5d" opacity="0.92" />
                    <rect x="38" y="64" width="18" height="40" rx="5" fill="#78b7b1" opacity="0.9" />
                    <rect x="60" y="48" width="18" height="56" rx="5" fill="#dcefe9" opacity="0.96" />

                    <path d="M18 103 Q32 90 46 97 T72 100 T87 88" fill="none" stroke="#0e5a5d" strokeWidth="3" strokeLinecap="round" />
                    <circle cx="25" cy="95" r="3.5" fill="#0e5a5d" />
                    <circle cx="48" cy="97" r="3.5" fill="#4da59d" />
                    <circle cx="71" cy="99" r="3.5" fill="#8cc2b9" />
                    <circle cx="86" cy="88" r="3.5" fill="#0e5a5d" />

                    <rect x="16" y="116" width="40" height="8" rx="4" fill="#edf2f1" />
                    <rect x="60" y="116" width="22" height="8" rx="4" fill="#d9efec" />
                  </g>

                  <g transform="translate(76 26) rotate(8 42 74)">
                    <rect x="0" y="0" width="82" height="108" rx="10" fill="#ffffff" stroke="#dfeae9" />
                    <rect x="12" y="14" width="54" height="8" rx="4" fill="#edf4f3" />
                    <rect x="12" y="28" width="36" height="7" rx="4" fill="#e9f4f3" />

                    <path d="M12 76 L26 68 L38 72 L52 56 L62 60 L70 44" fill="none" stroke="#1a7675" strokeWidth="2.6" strokeLinecap="round" strokeLinejoin="round" />
                    <circle cx="52" cy="56" r="3.5" fill="#1a7675" />

                    <rect x="12" y="84" width="12" height="14" rx="3" fill="#d7eee8" />
                    <rect x="28" y="80" width="12" height="18" rx="3" fill="#75b5ae" />
                    <rect x="44" y="72" width="12" height="26" rx="3" fill="#165f60" />
                    <rect x="60" y="64" width="12" height="34" rx="3" fill="#8cc2b9" />
                  </g>
                </svg>
              </div>
          ...
          {loading ? (
            ...
            <div className="mt-5 grid gap-4 md:grid-cols-2">
              {history.map((batch) => {
                ...
                const chipClass = getStatusClasses(batch.status);
                ...
                return (
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <span className={`inline-flex shrink-0 items-center rounded-full border px-2 py-1 text-[11px] font-medium ${chipClass}`}>
