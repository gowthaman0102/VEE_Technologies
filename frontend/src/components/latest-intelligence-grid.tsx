...
export function LatestIntelligenceGrid({ items }: { items: DashboardIntelligenceItem[] }) {
  return (
    <div className="w-full rounded-[16px] bg-surface border border-border shadow-[0_8px_24px_rgba(23,54,76,0.04)] p-5 lg:p-6 mb-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6">
        ...
        <Link 
          href="/intelligence" 
          className="mt-4 sm:mt-0 inline-flex shrink-0 items-center justify-center rounded-[10px] border border-border bg-surface px-4 py-2 text-[13px] font-bold text-text transition-colors hover:bg-surface-raised hover:border-primary"
        >
          View all
        </Link>
      </div>
