...
export function SearchPageClient() {
  ...
  async function submit(
    ...
    try {
      ...
      setReportPeriod({
        start: filters.start ?? new Date(0).toISOString(),
...
function SearchResultsModal({
  ...
  async function createReport() {
    if (companyId === null || !reportPeriod || filtered.length === 0) return;
    setGenerating(true);
    try {
      await generateReport({
        company_id: companyId,
        report_type: mode === "keyword" ? "search_keyword" : "search_semantic",
        report_scope: "search",
        report_title: `${mode === "keyword" ? "Keyword" : "Semantic"} search: ${query}`,
        time_mode: "media",
        start_date: reportPeriod.start,
        end_date: reportPeriod.end,
        article_ids: filtered.map((article) => article.article_id),
      });
      toast.success("Search report generated. It is available in Search reports.");
      window.location.href = "/reports?section=search";
    } catch (cause) {
      toast.error(cause instanceof Error ? cause.message : "Unable to generate search report.");
    } finally {
      setGenerating(false);
    }
  }
        ...
        <header className="flex shrink-0 items-start justify-between gap-4 border-b border-border px-5 py-4 sm:px-6">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={`${primaryButton} min-h-9 rounded-lg px-3`}
            >
